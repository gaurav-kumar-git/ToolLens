...
OUTPUT_DIR = './pos_corr_question_anchor_protected_2'
...
for q_id in unique_qids:
    ...
    for pos in tqdm(range(NUM_DBS), desc=f"Positional Sweep"):
        ...
        # import ipdb; ipdb.set_trace()
        pos_raw_density  = np.zeros((80, 32, 32))  # [db_idx, layer, head]
        pos_corr_density = np.zeros((80, 32, 32))
        ...
        if q_id in QUERIES_TO_PLOT:
            others = [i for i in range(80) if i != gold_i]
